...
-   Functions:
    ...
    -   If the object is in the name, prefer a single generic object name rather
        than multiple specific names: `read_csv()` rather than `read_csv_file()` or 
        `upload_file()` rather than `upload_csv_file()` (uploading isn't depending 
        on file extension, while reading is)
