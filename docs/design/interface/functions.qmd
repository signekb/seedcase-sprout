...
optionally the path to the `datapackage.json` file and will read in the
data resource (as a Parquet file) into a Polars DataFrame. If the `path`
is not given, it will look for the `datapackage.json` file in the
