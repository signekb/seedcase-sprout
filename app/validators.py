...
def validate_no_special_characters(
    ...
    """validate_no_special_characters Validation check for special characters.

    Checks that a field does not include special characters, i.e., characters
    that are not considered numbers or letters.

    Args:
        field_name (str): Name of the field that should be included in the error
        message.
...
def validate_table_name_does_not_exist(name: str) -> ValidationError | None:
    """validate_table_name_does_not_exist Validation check for if table name exists.
