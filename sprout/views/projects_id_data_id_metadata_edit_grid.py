...
def projects_id_data_id_metadata_edit_grid(request, table_id):
    ...
    if request.method == "POST":
        ...
        if all(form.is_valid() for form in forms):
            for form in forms:
                form.save()
            return redirect(
                reverse("projects-id-data-id-metadata-edit-grid", args=[table_id])
            )
    ...
    return render(
        request,
        "projects-id-data-id-metadata-edit-grid.html",
        {
            "forms": forms,
