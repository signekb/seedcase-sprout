...
ENTRYPOINT ["/bin/bash", "-c" , "poetry run python manage.py collectstatic --no-input  && poetry run python manage.py migrate && poetry run gunicorn config.wsgi --bind 0.0.0.0:8000"]
